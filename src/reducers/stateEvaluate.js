...
function stateEval(state, action) {
    ...
    switch (state.seq) {
        case 0:
            if (state.chckrLocations[action.payload] === state.turn || state.chckrLocations[action.payload] === (state.turn + " King")) //if its your turn and you select one of your own checkers
            {
                newState.highlighted = [];
                newState.highlighted.push(action.payload);      //highlight selected checker
                newState.highlighted.push(...getMoves(action.payload, state.chckrLocations));   //highlight its moves

                let j = [];
                j = getJump(state.chckrLocations[action.payload], action.payload, state.chckrLocations, j);
            ...
            return newState;
        case 1:
            if (state.chckrLocations[action.payload] === state.turn || state.chckrLocations[action.payload] === (state.turn + " King")) { //if you reselect a one of your own checkers
                newState.highlighted = [];
                newState.highlighted.push(action.payload);      //highlight selected checker
                newState.highlighted.push(...getMoves(action.payload, state.chckrLocations));   //highlight its moves
            ...
            else if (state.highlighted.includes(action.payload)) { //if its your turn, you've select a valid checker already, and you've selected a valid square to move to
